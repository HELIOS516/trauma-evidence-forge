...
const OUTPUT =
  "/Users/fee4jk/Library/CloudStorage/OneDrive-UniversityofVirginia/LECTURES/Trauma-Shelf-Review-2026.pptx";
...
function parseMarkdown(md) {
  ...
  for (const raw of rawSlides) {
    ...
    if (
      ksMatch &&
      !ksMatch[1].startsWith("Sources") &&
      !ksMatch[1].startsWith("Gamma")
    ) {
      keyStat = ksMatch[1];
    }
    ...
    if (title.includes("Clinical Decision Point")) type = "mcq";
      ...
      title === "Trauma for the Shelf: Every Topic in 45 Minutes" &&
      !slides.length
    )
      type = "title";
    else if (title.includes("Objectives") || title.includes("objectives"))
      type = "objectives";
    else if (title === "References") type = "references";
    else if (title.includes("Ten Topics, Ten Discriminators"))
      type = "takehome";
    ...
    switch (s.type) {
      ...
      case "mcq":
        ...
        for (const l of qLines) {
          if (l.trim().startsWith("- A.") || l.trim().startsWith("- A)"))
            inOptions = true;
          if (inOptions) optionLines.push(l.trim().replace(/^- /, ""));
          else if (!l.includes("-->") && !l.includes("Speaker Notes"))
          ...
          const tableRows = tbl.map((row, ri) =>
            row.map((cell) => ({
              ...
              options: {
                ...
                bold: ri === 0,
                fill: {
                  color: ri === 0 ? ACCENT : ri % 2 === 0 ? "0D2137" : NAVY,
                },
